describe('the Vector3 object', function () {
    ...
    describe('Vector3 tests', function () {
        ...
        it('transforms the vector using a matrix', function () {
            var matrix = new Matrix33([1, 1, 1, 1, 1, 1, 1, 1, 1]);
            var vec = new Vector3(1, 1, 1);
            var result = vec.transformed(matrix);
            expect(result.x).not.toBeNull; //Näistä tulee NaN, vaikka pitäisi olla numero
            expect(result.x).not.toBeNull; //väliaikaisesti testataan vain ettei ole null.
